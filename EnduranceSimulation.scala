...
class EnduranceSimulation extends Simulation {
  val duration = Duration(System.getProperty("duration").toInt / 4, "seconds")
  ...
  val sim = scenario(s"Endurance (Users=$users - Duration=$duration)")
      .exec(
   ...
   setUp(
      sim.inject(
         rampUsersPerSec(1) to (users * 0.8) during duration,
